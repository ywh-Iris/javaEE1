...
//		Analyzer analyzer = new IKAnalyzer();
		...
		for (File file : files) {
			...
			// 7、对应文件的属性创建Field对象，添加到Document对象中。
			//参数1：域的名称 参数2：域的值 参数3：是否存储，如果存储可以取出，如果不存储不能取出。不影响分词创建索引。
...
//			Field fieldSize = new LongField("size", fileSize,  Store.YES);
	...
	public void searchIndex() throws Exception {
		...
		Directory directory = FSDirectory.open(new File("D:/temp/JavaEE71/index"));
		// 2）创建一个IndexReader对象
